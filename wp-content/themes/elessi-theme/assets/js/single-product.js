...
$('body').on('click','.nasa-wrap-review-thumb .nasa-item-review-thumb,.nasa_reviewed_images .nasa-item-reviewed-thumb', function(e) {
    ...
    gallery.listen('afterChange', function() {
        ...
        $.each(_thumbs, function( index, value ) {
            if (!$(this).find('.nasa-wrap-review-thumb').hasClass('slick-initialized')) {
    ...
    var silde = $(pswpElement).find('.nasa-slick-slider[data-id="'+ _pa_id +'"]');
    if($(silde).length) {
        $(silde).slick('slickGoTo',media_key);
    }
});
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.code == 'success') {
                    ...
                    _url.searchParams.delete('review_sort');
                    window.history.pushState(_url.href, '', _url.href);
