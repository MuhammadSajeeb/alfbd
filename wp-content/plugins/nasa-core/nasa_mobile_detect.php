 ...
 * Only Front-end
 */
function nasa_check_is_phone() {
    global $nasa_is_phone;
    
    if (!isset($nasa_is_phone)) {
        $nasa_mobile_detect = new Nasa_Mobile_Detect();
        
        $nasa_is_phone = $nasa_mobile_detect->isMobile() && !$nasa_mobile_detect->isTablet() ? true : false;
        
        /**
         * Support - WP Rocket | Set Tablets As Mobile plugin
         */
        if (function_exists('tablets_as_mobiles') && function_exists('wp_is_mobile')) {
            $nasa_is_phone = wp_is_mobile();
        }
        
        $GLOBALS['nasa_is_phone'] = apply_filters('ns_is_phone', $nasa_is_phone);
    }
    
    return $nasa_is_phone;
}
