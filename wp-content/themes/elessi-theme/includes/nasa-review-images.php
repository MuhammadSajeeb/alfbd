            ...
            foreach ($first_three_attachments as $key => $id_media){
                ...
                if($url) {
                    if (in_array($type_file, array("image/jpg", "image/jpeg", "image/bmp", "image/png", "image/gif", "image/webp"))) {
                        $media = $firstmedia_class == 'nasa-first-media-review' ? wp_get_attachment_image($id_media, 'full', false, array('class' => 'skip-lazy attachment-full size-full')) : wp_get_attachment_image($id_media, apply_filters('nasa_reivew_product_thumbnail_size', 'fill'), false, array('class' => 'skip-lazy attachment-thumbnail size-thumbnail'));
                    } else {
                        $media = '<video src="' . esc_url($url) . '"><source src="' . esc_url($url) . '" type="'. $type_file .'"></video>';
                ...
                <a  data-index="<?php echo $key ;?>" 
                    ...
                    class="nasa-item-review-thumb <?php echo( $firstmedia_class );?>" 
